function disp = ssd(src1,src2,window_size,disp_threshold,row)
...
for i=1:w % Center pixel of window1, which is from src1 (reference)
    ...
    for j=1:w % Center pixel of window2, which is from src2 (object being probed)
        ...
        windows_diff = window1-window2;
        windows_diff_sq = windows_diff .* windows_diff;
        sum_val = sum(windows_diff_sq(:));
